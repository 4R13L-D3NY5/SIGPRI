...
export function ProposalTutorialModal({
  ...
  // Pestaña 1 por defecto: "pasos" (Guía del paso a paso)
  const [modalTab, setModalTab] = useState<"pasos" | "diagrama" | "requisitos">("pasos");
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 sm:p-6 overflow-y-auto animate-in fade-in duration-200">
      <div className="relative my-auto w-full max-w-5xl bg-card border border-border rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[88vh] animate-in zoom-in-95 duration-200">
        ...
        {/* PESTAÑAS PRINCIPALES DEL TUTORIAL CON NUEVO ORDEN Y NOMBRES */}
        <div className="px-6 py-2.5 bg-background border-b border-border flex items-center justify-between gap-2 overflow-x-auto shrink-0">
          <div className="flex items-center gap-2">
            ...
            {/* PESTAÑA 1: GUÍA DEL PASO A PASO */}
            <button
              type="button"
            ...
            >
              ...
              <span>📖 1. Guía del paso a paso</span>
            </button>

            {/* PESTAÑA 2: DIAGRAMA DE SECUENCIA */}
            <button
              type="button"
            ...
            >
              ...
              <span>📊 2. Diagrama de secuencia</span>
            </button>

            {/* PESTAÑA 3: REQUISITOS Y PRESUPUESTOS */}
            <button
              type="button"
            ...
            >
              ...
              <span>📋 3. Requisitos y presupuestos</span>
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {/* ========================================================= */}
          {/* PESTAÑA 1: GUÍA DEL PASO A PASO (NUEVA PESTAÑA 1) */}
          {/* ========================================================= */}
          {modalTab === "pasos" && (
            <div className="space-y-5 animate-in fade-in duration-200">
              <div className="flex items-center justify-between border-b border-border pb-2">
                <h3 className="text-xs font-extrabold uppercase tracking-wider text-foreground flex items-center gap-2">
                  <Layers className="h-4 w-4 text-primary" /> Guía del Paso a Paso del Proceso de Postulación
                </h3>
              ...
              <div className="p-5 rounded-2xl border border-border bg-card space-y-4 shadow-sm">
                ...
                {/* PASO 3 MODIFICADO (SIN ANEXO III, SIN PAT UNITEPC, SIN APA 7, SIN WBS) */}
                {activeStepTab === 3 && (
                  <div className="space-y-3 animate-in fade-in duration-150">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      En la tarjeta de la propuesta registrada, haga clic en los 3 botones principales para completar la formulación:
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1">
                      <div className="p-3.5 rounded-xl border border-primary/30 bg-primary/5 space-y-1">
                        <div className="font-bold text-xs text-primary flex items-center gap-1">
                          <FileText className="h-3.5 w-3.5" /> 📄 1. Detalle de la Propuesta
                        </div>
                        <p className="text-[11px] text-muted-foreground leading-snug">
                          Formulación científica completa (Planteamiento del Problema, Objetivos, Metodología, Impactos) y equipo de coinvestigadores.
                        </p>
                      </div>
                      ...
                      <div className="p-3.5 rounded-xl border border-amber-500/30 bg-amber-500/5 space-y-1">
                        <div className="font-bold text-xs text-amber-500 flex items-center gap-1">
                          <Calendar className="h-3.5 w-3.5" /> 📅 2. Cronograma de Actividades
                        </div>
                        <p className="text-[11px] text-muted-foreground leading-snug">
                          Plan de trabajo estructurado por fases secuenciales, entregables esperados, responsable y fechas proyectadas.
                        </p>
                      </div>
                      ...
                      <div className="p-3.5 rounded-xl border border-emerald-500/30 bg-emerald-500/5 space-y-1">
                        ...
                        <p className="text-[11px] text-muted-foreground leading-snug">
                          Tabla de gastos indicando FACTURA o RETENCIÓN impositiva de ley (Servicios 15.5%, Bienes 8%, Alquileres 16%) con cálculo automático.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {activeStepTab === 4 && (
                  <div className="space-y-3 animate-in fade-in duration-150">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      La propuesta es enviada a revisión y pasa a <code>🔍 2. En Evaluación</code>. Los comités evalúan la pertinencia, calidad científica y aspectos éticos emitiendo un dictamen formal.
                    </p>
                  </div>
                )}
                ...
                {activeStepTab === 5 && (
                  <div className="space-y-3 animate-in fade-in duration-150">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      Con dictamen favorable y visto bueno contable, la propuesta pasa a <code>🚀 4. Aprobado en Ejecución</code>, habilitando el desembolso de fondos y la fiscalización del cronograma.
                    </p>
                  </div>
                )}
              </div>
          ...
          {/* ========================================================= */}
          {/* PESTAÑA 2: DIAGRAMA DE SECUENCIA (NUEVA PESTAÑA 2 - SIN WBS) */}
          {/* ========================================================= */}
          {modalTab === "diagrama" && (
              ...
              <div className="p-5 rounded-2xl border border-primary/30 bg-muted/20 space-y-4 shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border/60 pb-3">
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">Flujo secuencial gráfico desde la creación de cuenta hasta la aprobación y desembolso</p>
                  </div>
                  <Button
                    type="button"
                    variant="outline"
                ...
                <div className={`w-full overflow-x-auto rounded-xl border border-border bg-slate-950 p-4 shadow-2xl transition-all ${zoomSvg ? "max-h-[75vh]" : ""}`}>
                  ...
                  >
                    ...
                    {/* PASO 3: COMPLETAR 3 APARTADOS (DETALLE, CRONOGRAMA, PRESUPUESTO - SIN WBS) */}
                    <g transform="translate(410, 35)">
                      ...
                      <text x="80" y="88" textAnchor="middle" fill="#93C5FD" fontSize="8.5" fontWeight="bold">📄 1. Detalle Propuesta</text>

                      <rect x="15" y="102" width="130" height="22" rx="6" fill="#1E293B" stroke="#F59E0B" strokeWidth="1" />
                      <text x="80" y="116" textAnchor="middle" fill="#FDE047" fontSize="8.5" fontWeight="bold">📅 2. Cronograma Trabajo</text>
                    ...
                    {/* RESUMEN DE LEYENDA ABAJO (LIMPIADO SIN WBS, APA 7, ANEXO III, PAT UNITEPC) */}
                    <g transform="translate(30, 335)">
                      <rect x="0" y="0" width="920" height="75" rx="12" fill="#0F172A" stroke="#334155" strokeWidth="1" />
                      <text x="20" y="24" fill="#38BDF8" fontSize="11" fontWeight="bold">💡 Leyenda de Flujo y Requisitos Universitarios UNITEPC:</text>
                      <text x="20" y="43" fill="#94A3B8" fontSize="10">• Detalle de la Propuesta: Formulación científica completa (Planteamiento del Problema, Objetivos, Metodología, Resultados) y Equipo con C.I.</text>
                      <text x="20" y="60" fill="#94A3B8" fontSize="10">• Cronograma de Actividades y Presupuesto Financiero (Facturas o Retenciones impositivas de Servicios / Bienes / Alquileres).</text>
                    </g>
          ...
          {/* ========================================================= */}
          {/* PESTAÑA 3: REQUISITOS Y PRESUPUESTOS (NUEVA PESTAÑA 3) */}
          {/* ========================================================= */}
          {modalTab === "requisitos" && (
            <div className="space-y-5 animate-in fade-in duration-200">
              <div className="p-5 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 space-y-4">
                <h3 className="text-xs font-extrabold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 flex items-center gap-2">
                  <FileText className="h-4 w-4" /> Checklist de Requisitos y Presupuestos UNITEPC
                </h3>
